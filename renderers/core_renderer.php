...
class theme_vision_core_renderer extends theme_essential_core_renderer {
    ...
    public function footer() {
        ...
        // if single-sign-on is set then create an iframe for the form
        if(isset($_SESSION['single-sign-on'])){
            $sso_url = (!empty($_SERVER['HTTPS']))
            ? "https://".$_SERVER['SERVER_NAME']."/my/"
                    : "http://".$_SERVER['SERVER_NAME']."/my/";
            $footer .= "\n<br><iframe height='1px' width='1px' src='".$sso_url."?single-sign-on'></iframe><br>\n";
        }
        
        //sigle sign off of myPay and alfresco
        if(isset($_GET['single-sign-off'])){
            $eo_server = ($_SERVER['HTTP_HOST'] == 'staffdev.hcpss.org') ? "staffdev.hcpss.org" : "eo.hcpss.org";
            $footer .= "<img src=\"https://teach.hcpss.org/share/page/dologout\"></img>
            <img src=\"https://$eo_server/employeeonline/eoexit.asp\"></img>";
        }
